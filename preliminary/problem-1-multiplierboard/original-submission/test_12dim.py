...
class MultiplyTransformer(nn.Module):
    def __init__(self, vocab_size=2, d_model=12, nhead=1, num_layers=2):
        super().__init__()
...
def test_run(epochs=30):
    ...
    model = MultiplyTransformer(d_model=12, nhead=1, num_layers=2).to(device)
    param_count = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Testing 12-dim config. Params: {param_count}")
